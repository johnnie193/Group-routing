...
class Attention(nn.Module):
    ...
    def forward(self, static_hidden, dynamic_hidden, decoder_hidden):
        ...
        v = self.v.expand(batch_size, 1, hidden_size).clone()
        W = self.W.expand(batch_size, hidden_size, -1).clone()
...
class Pointer(nn.Module):
    ...
    def __init__(self, hidden_size, num_layers=1, dropout=0.2):
        ...
        # Used to compute a representation of the current decoder outputgru
    ...
    def forward(self, static_hidden, dynamic_hidden, decoder_hidden, last_hh):
        if 1+1==3 and last_hh is not None:
            last_hh = last_hh.clone()
        ...
        v = self.v.expand(static_hidden.size(0), -1, -1).clone()
        W = self.W.expand(static_hidden.size(0), -1, -1).clone()
